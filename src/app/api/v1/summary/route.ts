// =============================================================================
// CFO Family Finance App — Summary API Route (GET)
// =============================================================================
...
// GET /api/v1/summary — Dashboard summary (min role: MEMBER)
export const GET = withFamily(async (_req, ctx) => {
    try {
        const roleCheck = requireRole('MEMBER')(ctx)
        if (roleCheck) return roleCheck

        const result = await SummaryService.getDashboardSummary(ctx.familyId)
        return ok(result)
    } catch (error) {
